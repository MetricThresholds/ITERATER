...
File: RQ2_random_forest_logit_on_al_test_apv.py
...
def random_forest_on_tal_test(working_dir, result_dir):
    ...
    dir_spv_rf = result_dir + 'randomForest/apv_20_percent/'
    dir_spv_logit = result_dir + 'logit/apv_20_percent/'
    ...
    for project_t in projects_t:
        ...
        for name in versions_sorted:
            ...
            if name == versions_sorted[len(versions_sorted) - 1]:
                ...
                # print(df_name_previous.head())
                # print(len(df_name_previous))
                # print(df_name_test.head())
                # print(len(df_name_test))
                # df_name_previous['bugBinary']
                print(len(df_previous_all))

                # 随机森林分类模型（RandomForestClassifier）与随机森林回归模型（RandomForestRegressor）
                df_rf_x = df_previous_all.loc[:, metrics_20]
                df_rf_y = df_previous_all.loc[:, 'bugBinary']
                # print(df_rf_y)
                # print(df_rf_x)
                ...
                df_random_forest = df_random_forest.append(
                    {'Sample_size': len(df_name_test), 'project': name.split('-')[0], 'last_version': name_previous_all,
                ...
                # clf = LogisticRegression(random_state=0).fit(df_rf_x, df_rf_y)
                # logit分类预测
                for row in range(len(df_name_test)):
